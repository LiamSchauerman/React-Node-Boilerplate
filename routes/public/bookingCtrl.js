...
app.controller('bookingCtrl', function ($scope, $http, $window, $document, $location) {
  ...
  var currentRes = {};
      ...
      $scope.submitThis = function($event, acc){
        ...
        $scope.rental.startDate = moment($scope.rental.dateRange.slice(0, $scope.rental.dateRange.indexOf('-') - 1), "MM DD YYYY").format('X')
        $scope.rental.endDate = moment($scope.rental.dateRange.slice($scope.rental.dateRange.indexOf('-') + 2, $scope.rental.dateRange.length), "MM DD YYYY").format('X')
